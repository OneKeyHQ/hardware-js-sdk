...
2. **Branch handling**
   - **If on `onekey` branch**: Create a new feature branch first
   ...
   - **If already on a feature branch (not `onekey`)**: Skip branch creation, proceed directly to commit
...
- Always target `onekey` as the base branch for the PR
