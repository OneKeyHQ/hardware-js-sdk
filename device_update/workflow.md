...
- 6.执行 firmware update 指令，类型为 3，路径为 vol0:update_rom.bin，执行成功下进行下一步，否则返回第一步
...
- 4.执行 firmware update 指令，类型为 6，路径为 vol0:bluetooth.bin，这里不判断firmware update执行结果，直接等待 FirmwareInstallProgress 返回 100%, 则认为成功，然后进入下一步
...
- 6.执行 firmware update 指令，类型为 4，路径为 vol0:core.bin，这里不判断firmware update执行结果，直接等待 FirmwareInstallProgress 返回 100%, 则认为成功，然后进入下一步
