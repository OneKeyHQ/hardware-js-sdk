...
用固件默认测试助记词 `abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about` 的三个测试账户生成 P2SH、P2SH-P2WSH、P2WSH 的 2-of-3 fixture。每类包含 `btcGetAddress` 和 `btcSignTransaction`；交易引用必须与 multisig scriptPubKey、prev_hash 和金额一致。用例设置 `testMnemonicOnly: true`。
