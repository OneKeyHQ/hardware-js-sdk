...
- **阶段 2（QR 事件循环 + Adapter 骨架）已完成**：`KeystoneAdapter` 实现完整 `IHardwareWallet`
  ...
  - **冷启动两跳流程已实现**：`connectId`/`deviceId` 为空时，签名类方法先隐式发一次 `qr-hardware-call`
    KeyDerivation 请求，同时拿到 8 HEX mfp 与固定身份 xpub，再发真正的签名请求——对调用方就是一次普通方法
    调用，内部按需 1~2 次二维码往返。已同步过的钱包（`_devices` 按 64 HEX wallet id 缓存）跳过第一跳。
  - **两种缓存粒度分开处理**：签名类方法（`evmSignTransaction` 等）只需要钱包记录，走 `_ensureWalletKnown`
  ...
  - **2026-08-25 修正**：`_ensureWalletKnown` 曾直接把 `CHAIN_FINGERPRINT_PATHS.evm`（5 段叶子路径
...
- **阶段 3（USB connector）已完成**：`packages/hwk-keystone-connector-usb`——`IConnector` 从零实现
  ...
  `TransportNodeUSB`）。要点：
  - `connect()` 打开 transport 后立刻 `CMD_GET_DEVICE_VERSION` 拿 `walletMFP`，但只把它放进
    `raw.masterFingerprint`；`sessionId` 使用统一 runtime-ID helper 生成的进程内随机 opaque 会话键，connector 的 `deviceId` 保持空。
    Adapter 随后通过固定身份 xpub 派生稳定 wallet id。mfp 仅用于协议预检，不再承担唯一身份。
  ...
  - WebUSB exact-open 注册的全局 disconnect listener 由 transport-lifetime cleanup 管理；错误候选、
    session disconnect、reset/dispose 与物理拔线都会幂等释放，不随反复探测累计。
  - `cancel()`/`uiResponse()` 均为文档化的 no-op——USB 无协议级取消，PIN/passphrase 全部走设备触屏。
  - connector 单测覆盖 search/connect、exact descriptor、selection/availability 双快照代次、过期 token、
    resolveUr/checkLockStatus 往返（真实 UR 编解码）、错误映射、未知 session 与 reset。
- 显式 `transportType:'usb'` 的扫描会把 permission/enumeration 错误交给宿主；只有未指定通道的聚合
  扫描会在 USB 扫描失败时保留已知 QR wallet，而不会把 USB 故障伪装成“0 台设备”。
- **阶段 4（双通道归并与路由）已完成**：`KeystoneAdapter` 构造函数新增可选 `usbConnector: IConnector`
  ...
  - `KeystoneDeviceRecord` 新增 `usbSessionId`/`qrSynced`/`hadUsbSession` 字段；`_resolveUr()` 是唯一的通道路由
  ...
  - **两条通道统一显式 onboarding，QR 仍支持隐式冷启动**：`searchDeviceTargets({transportType:'qr'})` 只返回
    一个虚拟连接目标，不触发 QR 或账户导出；随后的 `connectDevice()` 执行一次身份 KeyDerivation 往返，
    完成钱包身份同步并返回 runtime-only `interactionId`。链方法继续支持 `connectId`/`deviceId` 传 null 的
    隐式冷启动。USB 侧因为 WebUSB 拾取器要求用户手势，`searchDeviceTargets()` 只做描述符级枚举
    （不 open+claim，不产生 wallet id），同样由 `connectDevice()` 打开设备并完成身份同步；账户派生延迟到
    后续业务调用，并复用该 Interaction 的既定 USB 通道。
  - **按 wallet id 归并，不用 `createCombinedConnector`**（该工具明确不去重，服务的是"一厂两传输各自成条目"
    场景，与本方案相反）：`connectDevice()` 派生 wallet id 后，若 `_devices` 已有同 wallet id 的 QR 条目就原地升级
    （emit `device-changed`，不是第二次 `device-connect`）；否则新建一条 USB-only 条目。
  - `releaseInteraction()` 或 USB 被动断开：若该 wallet id 曾经 QR 同步过（`record.qrSynced`）→ 降级为 QR-only 条目
    （`device-changed`）；从未 QR 同步过的纯 USB 条目 → 整条移除（`device-disconnect`）。
  - `_ensureAccountSynced`/`_ensureWalletKnown` 在目标钱包已有 USB session 时，其隐式 KeyDerivation 同步
    也会走 USB（不再局限于签名/取地址请求本身）——但 USB `connect()` 本身已经用 `getAppConfig` 学到了
    mfp，所以 `_ensureWalletKnown` 通常直接命中缓存，完全不需要额外同步往返（比 QR 冷启动更快一跳）。
  - `DeviceInfo.raw.availableChannels`（新增，非核心类型改动，纯 `raw` 透传字段）在双通道归并后同时列出
    `['qr','usb']`，供 App 展示。
  - Adapter 回归覆盖 USB-only 新建、QR+USB 归并、通道 pin、断开降级、多设备 exact selection、冷启动
    按 full wallet id 逐个验证、签名不重放、reset/dispose drain；使用真实 UR 编解码 fake `IConnector`，
    不 mock adapter 内部。
  - 真机验证：QR 导入 → 插 USB 签名 → 拔线 QR 签名的完整闭环仍未做（USB 全链路只能真机验证）。
- **2026-08-31 身份与包边界修正**：8 HEX mfp 不再同时充当 `deviceId`、`connectId`、USB session 与
  `_devices` Map key。SDK 用 `m/44'/60'/0'` 的固定账户级 xpub，经带域分隔的 SHA-256 派生 64 HEX
  wallet id；QR/USB 均请求同一身份源。开发阶段不保留 `keystone-qr:<8 HEX>` 或裸 8 HEX 查找；所有
  逻辑钱包查找只接受 64 HEX wallet id。`hw-transport-nodeusb` 同时从 connector 的必装 dependency
  移为 optional peer（SDK 开发侧保留 devDependency），仅使用 `/webusb` 的 App 不再安装原生 `usb`。
- **冷启动体验结论**：首次 onboarding 的 `connectDevice()` 只请求固定身份 xpub；随后创建默认账户时按业务
  所需路径继续 KeyDerivation。App/background 重启后 adapter 内存记录丢失时，若 USB 可用，SDK 先逐个
  exact-open availability 候选并读取固定身份 xpub，命中持久 full wallet id 后再执行业务；未命中时才进入
  QR 身份同步/签名流程。这个额外身份步骤是安全边界，不能用 8 HEX MFP 或默认第一台替代。
...
| 标识                                        | QR 通道                                                                               | USB 通道                                                                                      | 结论                                              |
| ------------------------------------------- | ------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| **wallet id**                               | 固定请求 `m/44'/60'/0'` xpub 后派生                                                   | Adapter 连接时通过同一 KeyDerivation 请求派生                                                 | **唯一双通道可靠键；带域分隔的 SHA-256，64 HEX**  |
| mfp（种子主指纹, xfp）                      | `CryptoMultiAccounts.masterFingerprint`、`crypto-hdkey` 的 `origin.sourceFingerprint` | `getAppConfig().mfp`（walletMFP）、每次 `getPubkey/getURAccount` 都带                         | 8 HEX 协议字段，只做 xfp/快速预检；不可作为唯一键 |
...
**警示**：mfp 只有 8 HEX（4 bytes）。即使它来自种子，也会发生生日碰撞；它不能作为全局 `deviceId`、会话键或 Map
key。这里的「设备身份」定义为当前钱包（seed + passphrase）的稳定公共身份，不是物理 Keystone 单元。
...
| --------------------------------------- | ------------------------------------------- | ------------------------------------------------------------------------------- |
| `DeviceInfo.deviceId`                   | **wallet id（64 HEX，小写）**               | 固定身份 xpub 经带域分隔 SHA-256 派生；QR/USB 一致，换 seed/passphrase 必变     |
| `DeviceInfo.connectId`                  | `keystone-wallet:<walletId>`                | Adapter 对外的稳定逻辑钱包路由键；connector 内部另用 `keystone-usb-session:<n>` |
| `DeviceInfo.vendor`                     | `'keystone'`                                | 需扩宽 `VendorType`                                                             |
| `DeviceInfo.connectionType`             | `'qr'` 或 `'usb'`（当前调用实际使用的通道） | 需给 `ConnectionType` 增加 `'qr'`                                               |
| `capabilities.persistentDeviceIdentity` | Adapter `true` / connector `false`          | Adapter wallet id 持久；USB transport session 与枚举占位符不持久                |
| `getChainFingerprint`                   | 从稳定 wallet id 派生链域短指纹             | 与 Ledger 的短链指纹格式对齐，但不把短指纹反向当身份                            |
| QR 专属 `deviceId`（serial hash）       | 若某次 QR 同步刚好带了就存入 `raw`          | 仅 enrichment，**不参与身份判定**（USB 拿不到）                                 |

### 4.2 USB ↔ QR 归并规则

1. USB 插入并完成 `getAppConfig()` → 得到协议 mfp 与进程内 session；
2. Adapter 请求固定身份 xpub 并派生 wallet id；若已有同 wallet id 的 QR 条目则合并并 emit `device-changed`；
3. USB 拔出 → 条目退回 QR-only（若该 wallet id 曾经 QR 同步过）；纯 USB 条目则移除；
4. 64 HEX wallet id 与 App 目标不符时 fail-closed；8 HEX mfp 不参与查找或身份判定。

### 4.3 通道路由策略

默认 `auto`：**USB 可用且该链固件支持 USB 且 payload ≤ 帧上限 → USB；否则 QR**。宿主可用 `switchTransport('usb' | 'qr')` 显式锁定（现有接口，Trezor/Ledger 里是空 stub，Keystone 里第一次真正实现）。同一调用内通常**不做跨通道自动重试**——签名有副作用，USB 已送达设备后的失败交给 App，由用户决定换通道重发。唯一例外是 transport/host 在发送前返回 `PayloadTooLarge`：auto 模式可安全改走 QR；设备已回答或显式锁定 USB 时不降级。
...
sequenceDiagram
  ...
  App->>KA: searchDeviceTargets({transportType:'qr'})
  KA-->>App: virtual QR connection target（无设备交互）
  App->>KA: connectDevice(qrTarget) / allNetworkGetAddress(...)
  ...
  Note over KA: 解析 CryptoMultiAccounts → mfp + 固定身份 xpub<br/>派生 wallet id 并注册设备条目 → emit device-connect
...
sequenceDiagram
  ...
  Note over App,KA: 此前已通过 QR 导入，设备表: { deviceId: walletId, channels:['qr'] }
  App->>KA: (用户插入 USB 线) searchDevices()
  KA->>KA: USB enumerate → getAppConfig() → mfp 相同
  KA-->>App: device-changed { deviceId: walletId, channels:['qr','usb'] }
  App->>KA: btcSignPsbt(..., {interactionId})
  Note over KA: Interaction 固定走 USB；拔线后终止并返回错误，不在同一调用中切换 QR
```

## 6. API 对齐与错误语义

- **统一生命周期 API**：Ledger、Trezor、Keystone 都实现 `searchDeviceTargets()`、
  `connectDevice(searchTargetId) -> interactionId` 与 `releaseInteraction(interactionId)`。Ledger 连接时不会为了制造
  通用身份而额外请求任意链，首个实际链调用继续使用该链自己的 fingerprint 校验；Trezor 使用 Features
  的固件 `device_id` 验证物理设备，并以 passphrase state 区分钱包；Keystone 以固定身份 xpub 派生的
  64 HEX wallet id 验证逻辑钱包。Interaction ID 只绑定本次运行时连接，不替代这些身份材料。
...
| `src/types/wallet.ts:95-106`          | `AllNetworkDeviceIdentity` 增加 `vendor:'keystone'` / `type:'walletId'` 臂      |
...
| 2 ✅   | QR 通道 + Adapter 骨架   | `packages/hwk-keystone-adapter`：`KeystoneAdapter implements IHardwareWallet`、设备表（wallet id 键）、Job Queue、`REQUEST_QR_DISPLAY/SCAN` 循环、冷启动两跳同步、`qrTimeoutMs` 可配置超时；App-facing `importFromQr()` | 单测覆盖导入、冷/热启动签名、地址派生、签名、身份/mfp 不匹配、mfp 碰撞歧义、取消与超时。`hwk-demo` 真机联调未做——留给阶段 5                                                                            |
| 3 ✅   | USB connector            | `packages/hwk-keystone-connector-usb`：`IConnector` 从零实现，wrap `hw-transport-webusb`/`nodeusb`；锁定检查、超时、PRS*\*/ERR*\* 错误映射                                                                              | 单测 9 个（错误映射、mfp 不符 fail-closed、resolveUr/checkLockStatus 往返）全部通过；`hwk-demo` web 端真机 USB 签名**未做**（无物理设备）                                                              |
| 4 ✅   | 双通道归并与路由         | Adapter 内 wallet id 归并、`device-changed`、`switchTransport` 实装、`_resolveUr` 统一路由点                                                                                                                            | 单测覆盖合并/断开降级或移除/USB 优先路由/`switchTransport` 钉住；真机：QR 导入 → 插 USB 签名 → 拔线 QR 签名的完整闭环**未做**（USB 全链路只能真机验证）                                                |
...
| 身份     | 同 64 HEX wallet id 双通道归并；相同 8 HEX mfp 的不同 wallet id 并存；8 HEX 查找拒绝、USB 目标不符 fail-closed |
...
- 物理 `deviceId`（serial hash）未来若固件在 USB 路径补上，只可作为硬件 enrichment，不能替代 seed/passphrase 对应的 wallet id。

## 11. 待定决策（均给出默认建议）

1. **首批链**：建议 EVM + BTC + SOL + TRON（对齐现有 hwk 链接口全集）。
2. **USB 平台**：建议 WebUSB + NodeUSB 双发（Electron 桌面是 OneKey 主战场）。
3. **QR 设备持久化**：设备表由 App 持久化（wallet id + 协议 mfp + 已同步 xpubs），adapter 提供 import/export 快照接口;还是 adapter 自持存储?建议**App 持久化、adapter 无状态**，与 hwk 现有「adapter 不落盘」惯例一致。
