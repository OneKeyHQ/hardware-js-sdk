...
结论：**OnekeyFeatures 的 44 个字段已被 V2 的 `fw/bt/se1-4` 树完全覆盖**（version/build_id/hash/SE state/SE type 均有）。缺口集中在 V1 原始 `Features` 中的设备状态类语义；SDK 当前在标准 features 构建器（`deviceProfile/buildDeviceFeatures.ts`）中只能填占位值。
...
| bootloader*version, onekey_boot*\*                                    | `fw.boot.*`                                                                 |
| boardloader*version, onekey_board*\*                                  | `fw.board.*`                                                                |
...
3. `buildDeviceFeatures.ts` 中对应占位值（`unlocked: null`、`pinProtection: null` 等）替换为真实值。
