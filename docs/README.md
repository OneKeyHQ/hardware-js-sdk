...
中的接入说明。`docs/superpowers/` 下的阶段性设计和实施计划只作为历史背景，不是当前事实源。
...
| SDK  | [Pro2 无固件中间 Event 迁移](./sdk/pro2-eventless-migration.md)    | 历史 `DeviceSessionOpen` 迁移方案                |
...
- [阶段性设计与计划](./superpowers/README.md)只保留历史实施上下文，不覆盖当前事实。
...
5. 新的阶段性设计、实施计划和分支快照不进入长期事实文档，由 Git、提交记录和 PR
   保存；现有 `docs/superpowers/` 内容按其 README 作为历史资料保留。
