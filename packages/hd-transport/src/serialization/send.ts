...
export function buildOne(messages: protobuf.Root, name: string, data: Record<string, unknown>) {
...
export const buildEncodeBuffers = (messages: protobuf.Root, name: string, data: Record<string, unknown>) => {
  const { Message, messageType } = createMessageFromName(messages, name);
  const buffer = encodeProtobuf(Message, data);
  return encodeProtocol(buffer, {
...
export const buildBuffers = (messages: protobuf.Root, name: string, data: Record<string, unknown>) => {
  // const { Message, messageType } = createMessageFromName(messages, name);
  // const buffer = encodeProtobuf(Message, data);
