...
const LEDGER_MODEL_IDS = new Set(['nanoS', 'nanoSP', 'nanoX', 'stax', 'flex', 'apexp']);

export type LedgerRelayDevice = {
  id: string;
...
export type LedgerRelayApduErrorMessage = {
  type: 'apdu-error';
  requestId: string;
  message: string;
};

export type LedgerRelayClientMessage =
  | LedgerRelayHelloMessage
  | LedgerRelayApduResponseMessage
  | LedgerRelayApduErrorMessage;

export type LedgerRelayServerMessage =
  | {
      type: 'ready';
      protocolVersion: typeof LEDGER_ATTESTATION_RELAY_PROTOCOL_VERSION;
    }
...
const parseDevice = (value: unknown): LedgerRelayDevice => {
  ...
  return device as LedgerRelayDevice;
};

export const parseLedgerRelayClientMessage = (raw: string): LedgerRelayClientMessage => {
  if (Buffer.byteLength(raw, 'utf8') > MAX_LEDGER_RELAY_APDU_BYTES * 2 + 4_096) {
