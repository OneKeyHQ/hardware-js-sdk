...
import type { LedgerRelayApduBridge } from './relayTransport';
import type { LedgerRelayDevice } from './protocol';

export type LedgerServerGenuineCheckResult = {
  isGenuine: boolean;
  deviceId?: string;
};
  ...
  try {
    const discoveredDevice = await firstValueFrom(
      dmk
        ...
        .pipe(take(1), timeout({ first: 5_000 }))
    );
    ...
    return await new Promise<LedgerServerGenuineCheckResult>((resolve, reject) => {
      let settled = false;
