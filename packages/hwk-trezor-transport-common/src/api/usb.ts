import { arrayPartition, createDeferred, getSynchronize, resolveAfter } from '@onekeyfe/hwk-trezor-utils';
...
export class UsbApi extends AbstractApi {
    ...
    public async openDevice(...[path, options]: AbstractApiArgs<'openDevice'>) {
        ...
        // note: why for instead of scheduleAction from @onekeyfe/hwk-trezor-utils with attempts param. this.openInternal does not throw
    ...
    public async closeDevice(...[path]: AbstractApiArgs<'closeDevice'>) {
        ...
        device = this.findDevice(path);
        if (device?.opened) {
            const openedDevice = device;
            try {
                this.logger?.debug(`usb: device.close`);
                await this.synchronizeResetDevice(() => openedDevice.close());
