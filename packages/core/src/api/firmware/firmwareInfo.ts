import { Features, FirmwareRelease, ReleaseInfo, MajorVersion, StrictFeatures } from '../../types';
import { filterSafeListByFirmware, filterSafeListByBootloader, getScore } from './firmwareUtils';
  ...
  if (features.bootloader_mode) {
    ...
    if (features.firmware_present && features.major_version === 2) {
      ...
      return releases.filter(r =>
        versionUtils.isNewer(r.version, [features.fw_major, features.fw_minor, features.fw_patch])
      );
    }
...
export const getInfo = ({ features, releases }: GetInfoProps): ReleaseInfo | null => {
  ...
  if (major_version === 2 && bootloader_mode) {
    ...
    if (fw_major !== null && fw_minor !== null && fw_patch !== null) {
      ...
      parsedReleases = filterSafeListByFirmware(parsedReleases, [
        fw_major as unknown as MajorVersion,
        fw_minor,
        fw_patch,
      ]);
    }
    ...
    parsedReleases = filterSafeListByFirmware(parsedReleases, [
      major_version as unknown as MajorVersion,
      minor_version,
      patch_version,
    ]);
  }
  ...
  const changelog = getChangelog(parsedReleases, features as unknown as StrictFeatures);
  ...
  return {
    ...
    isNewer: isNewer(parsedReleases[0], features as unknown as StrictFeatures),
  };
};
