...
function hashOutpoint(hashWriter: HashWriter, input: NexaSignInputParams) {
...
function getPreviousOutputsHash(
  transaction: NexaSignTransactionParams,
...
function getSequencesHash(transaction: NexaSignTransactionParams, sighashType: SignatureType) {
  if (
...
function getSigOpCountsHash(transaction: NexaSignTransactionParams, sighashType: SignatureType) {
  if (isSighashAnyoneCanPay(sighashType)) {
    return zeroHash();
  }
...
function hashTxOut(hashWriter: HashWriter, output: NexaSignOutputParams) {
 ...
 *
 */
export function serialize(transaction: NexaSignTransactionParams, inputNumber: number) {
  const hashWriter = new HashWriter();
