...
export default class EVMSignTransaction extends BaseMethod {
  ...
  evmSignTx = async (address_n: number[], tx: EVMTransaction) => {
  ...
  evmSignTxEip1559 = async (address_n: number[], tx: EVMTransactionEIP1559) => {
    const {
      to,
      value,
      gasLimit,
  ...
  async run() {
    ...
    const formattedTx = formatAnyHex(tx);

    const signedTx = await (isEIP1559
      ? this.evmSignTxEip1559(address_n, formattedTx)
      : this.evmSignTx(address_n, formattedTx as EVMTransaction));
