...
export type DeviceFindMyTokenUpdateParams = {
  token: string;
};
...
const FIND_MY_TOKEN_MAX_BYTES = 256;
...
export function validateDeviceFindMyTokenUpdateParams(
  payload: Partial<DeviceFindMyTokenUpdateParams>
): DeviceFindMyTokenUpdateParams {
  return {
    token: validateHexBytes(payload.token, 'token', { maxBytes: FIND_MY_TOKEN_MAX_BYTES }),
  };
}
