import { TezosGetAddress as HardwareTezosGetAddress } from '@onekeyfe/hd-transport';
...
export default class TezosGetAddress extends BaseMethod<HardwareTezosGetAddress[]> {
  ...
  async run() {
    ...
    for (let i = 0; i < this.params.length; i++) {
      ...
      const res = await this.device.commands.typedCall('TezosGetAddress', 'TezosAddress', {
        ...param,
      });
