...
describe('Protocol V2 firmware update targets', () => {
  ...
  test('detects bootloader after an early App reply without reusing cached ProtocolInfo', async () => {
    ...
    try {
      ...
      const status = { initialized: true, unlocked: true };
      let protocolInfoRequests = 0;
      const typedCall = jest.fn((requestType: string) => {
        switch (requestType) {
          case 'DeviceReboot':
            return Promise.resolve({ type: 'Success', message: {} });
          case 'DeviceInfoGet':
            return Promise.resolve({ type: 'DeviceInfo', message: protocolV2BootloaderDeviceInfo });
          case 'ProtocolInfoRequest':
            protocolInfoRequests += 1;
            return Promise.resolve({
              type: 'ProtocolInfo',
              message:
                protocolInfoRequests === 1 ? protocolV2ApplicationInfo : protocolV2BootloaderInfo,
            });
          case 'DeviceStatusGet':
            return Promise.resolve({ type: 'DeviceStatus', message: status });
          default:
            throw new Error(`Unexpected request: ${requestType}`);
        }
      });
      device.commands = { typedCall } as unknown as DeviceCommands;
      device.updateProtocolV2Features(
        protocolV2BootloaderDeviceInfo,
        status,
        'normal',
        protocolV2ApplicationInfo
      );
      ...
      (method as any).protocolV2ExpectedSerialNumber = 'PR9999999999';
      (method as any).protocolV2ExpectedPath = 'usb-path';
      // A reused transport may answer before the application finishes rebooting.
      (method as any).reconnectProtocolV2Device = jest.fn().mockResolvedValue(undefined);
      method.postTipMessage = jest.fn();

      const result = method.enterProtocolV2BootloaderMode().then(
        value => ({ value }),
        error => ({ error })
      );
      for (let elapsed = 0; elapsed < 92_000; elapsed += 1000) {
      ...
      expect(typedCall.mock.calls.filter(([request]) => request === 'DeviceReboot')).toHaveLength(
        1
      );
      expect(typedCall).toHaveBeenNthCalledWith(
        1,
      ...
      expect(protocolInfoRequests).toBe(2);
      expect(device.isBootloader()).toBe(true);
      expect(method.postTipMessage).toHaveBeenLastCalledWith('GoToBootloaderSuccess');
    } finally {
      jest.clearAllTimers();
      jest.useRealTimers();
    }
