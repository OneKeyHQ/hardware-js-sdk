...
describe('DeviceCommands failure mapping', () => {
  it('selects an existing Protocol V1 Attach PIN wallet without opening Passphrase UI', async () => {
    const commands = createCommands();
    const promptPassphrase = jest.spyOn(commands, 'promptPassphrase');
    const commonCall = jest
      .spyOn(commands, '_commonCall')
      .mockResolvedValue({ type: 'PassphraseState', message: {} } as any);
    ...
    await expect(
      commands._filterCommonTypes(
        ...
        'GetPassphraseState',
        { passphraseAccess: 'attach-pin' }
      )
    ).resolves.toMatchObject({ type: 'PassphraseState' });

    expect(promptPassphrase).not.toHaveBeenCalled();
    expect(commonCall).toHaveBeenCalledWith(
      'PassphraseAck',
      { on_device_attach_pin: true },
      expect.objectContaining({ passphraseAccess: 'attach-pin' })
    );
  });

  it('does not expose Attach PIN inside the Protocol V1 Passphrase input flow', async () => {
    const commands = createCommands();
    const promptPassphrase = jest
      .spyOn(commands, 'promptPassphrase')
      .mockResolvedValue({ passphraseOnDevice: true });
    jest
      .spyOn(commands, '_commonCall')
      .mockResolvedValue({ type: 'PassphraseState', message: {} } as any);

    await commands._filterCommonTypes(
      {
        type: 'PassphraseRequest',
        message: { exists_attach_pin_user: true },
      } as any,
      'GetPassphraseState',
      { passphraseAccess: 'passphrase' }
    );

    expect(promptPassphrase).toHaveBeenCalledWith({ existsAttachPinUser: false });
  });

  it('exposes refreshed Protocol V1 Attach PIN capability in the formal App prompt', async () => {
    const commands = createCommands();
    const promptPassphrase = jest
      .spyOn(commands, 'promptPassphrase')
      .mockResolvedValue({ attachPinOnDevice: true });
    const commonCall = jest
      .spyOn(commands, '_commonCall')
      .mockResolvedValue({ type: 'PassphraseState', message: {} } as any);
    ...
    await commands._filterCommonTypes(
      ...
      'GetPassphraseState',
      { passphraseAccess: 'prompt' }
    );

    expect(promptPassphrase).toHaveBeenCalledWith({ existsAttachPinUser: true });
    expect(commonCall).toHaveBeenCalledWith(
      'PassphraseAck',
      { on_device_attach_pin: true },
      expect.objectContaining({ passphraseAccess: 'prompt' })
    );
  });

  it('logs wallet-session request bodies while redacting a host passphrase', async () => {
    const commands = createCommands();
    commands.mainId = 'main-id';
    commands.transport = {
      call: jest
        .fn()
        .mockResolvedValueOnce({ type: 'DeviceSession', message: {} })
        .mockResolvedValueOnce({ type: 'Success', message: {} }),
    } as any;
    const log = getLogger(LoggerNames.DeviceCommands);
    log.messages.length = 0;

    await commands.call('DeviceSessionGet', { session_id: 'visible-session-id' });
    await commands.call('DeviceSessionAskPassphrase', {
      passphrase: 'do-not-log-host-passphrase',
      on_device: false,
    });

    const serialized = JSON.stringify(log.messages);
    expect(serialized).toContain('visible-session-id');
    expect(serialized).toContain('DeviceSessionGet');
    expect(serialized).toContain('DeviceSessionAskPassphrase');
    expect(serialized).toContain('[REDACTED]');
    expect(serialized).not.toContain('do-not-log-host-passphrase');
  });
