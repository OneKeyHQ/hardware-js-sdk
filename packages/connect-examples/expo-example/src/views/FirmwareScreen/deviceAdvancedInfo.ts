...
const componentGroup = ({
  ...
  return {
    ...
    fields: [
      ...
      field(`${key}.hash`, hashLabelId, hash),
      ...extraFields,
    ],
  };
};
...
const securityElementGroup = (
  ...
  const applicationHashLabelId =
    state.protocol === 'V2'
      ? 'label__device_component_app_hash_prefix'
      : 'label__device_component_app_hash';
  const bootloaderHashLabelId =
    state.protocol === 'V2'
      ? 'label__device_component_boot_hash_prefix'
  ...
  return {
    ...
    fields: [
      ...
      field(`${key}.hash`, applicationHashLabelId, state.verification?.[hashKey]),
      ...
      field(`${key}.bootHash`, bootloaderHashLabelId, state.verification?.[bootHashKey]),
    ],
  };
};
...
export function buildDeviceAdvancedInfo(state: DeviceState): DeviceAdvancedInfo {
  ...
  const hashLabelId =
    state.protocol === 'V2'
      ? 'label__device_component_hash_prefix'
      : 'label__device_component_hash';
