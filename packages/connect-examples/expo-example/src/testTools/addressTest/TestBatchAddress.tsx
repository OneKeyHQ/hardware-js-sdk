...
function ExecuteView() {
  ...
  useEffect(() => {
    console.log('batchTestCases: ', batchTestCases);
  ...
  const beginTest = useCallback(async () => {
    ...
    for (const item of currentTestCases) {
      try {
        ...
        const isEmptyResult = Object.keys(item.result).find(
          key => isNil(item.result[key].address) || isEmpty(item.result[key].address)
        );
        if (isEmptyResult) {
