...
* `path` - _required_ `string | Array<number>` SDK minimum length is `5`. read more
...
The call below describes the current runtime contract. The current SDK TypeScript declaration incorrectly flattens the transaction fields and types `payment.amount` as a string; it does not match this runtime shape. Do not move `transaction` fields to the top level to satisfy that declaration.
