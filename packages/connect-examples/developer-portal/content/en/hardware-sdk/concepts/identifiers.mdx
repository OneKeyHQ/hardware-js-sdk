...
initialised. Firmware returns it to Core, but current public wallet-session
responses and the public Features projection do not expose it.
...
- Not surfaced to new app integrations; applications only need
  `deviceId + passphraseState`.
...
- Persist `deviceId + passphraseState` as the wallet identity. A new process
  explicitly opens the wallet again; it does not persist the internal session ID.
...
- New integrations persist `deviceId + passphraseState` as wallet identity
  and explicitly open the wallet in each new process. The SDK keeps
  `session_id` internal.
