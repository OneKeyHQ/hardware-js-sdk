...
- Persist `deviceId + passphraseState` as the wallet identity. The SDK keeps
  `session_id` private and restores it from its in-memory store.
...
| Role | SDK-internal device-session handle | Public hidden-wallet identifier |
