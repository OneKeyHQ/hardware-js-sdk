...
sequenceDiagram
  ...
  alt standard wallet
    ...
    App->>SDK: address / sign with useEmptyPassphrase true
  else hidden wallet
    ...
    App->>SDK: address / sign with passphraseState
  end
...
`openWalletSession` has no `resume-hidden` mode. Passing `mode: 'resume-hidden'` returns `CallMethodInvalidParameter`. Load the stored binding and pass `passphraseState` on the next address / sign call.
