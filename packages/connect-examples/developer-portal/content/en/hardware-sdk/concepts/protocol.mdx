...
Hardware SDK 1.2.0 talks two on-wire protocols. Integrators keep one JavaScript API.
...
| Device snapshot | `getFeatures()` (legacy) | Prefer `getDeviceState()` |
...
- `HardwareSDK.init({ env })` still selects the **transport**, not the protocol.
- `searchDevices` → `connectId` → `getFeatures` / `getDeviceState` → chain methods still works.
...
`getFeatures(connectId)` remains valid for existing apps. On Protocol V2 the SDK maps the live device into a compatibility Features object.

New code should prefer [`getDeviceState`](/en/hardware-sdk/basic-api/get-device-state):
