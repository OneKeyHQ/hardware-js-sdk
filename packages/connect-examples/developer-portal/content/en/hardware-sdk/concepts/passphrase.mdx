...
sequenceDiagram
    ...
    SDK->>Device: Derive on standard wallet
    Device-->>SDK: Address
    SDK-->>App: { address }
```
...
sequenceDiagram
    ...
    Note over App: On-device or software input
    App->>SDK: uiResponse(passphrase)
    SDK->>Device: Confirm hidden wallet
    Device-->>SDK: Ready
    SDK-->>App: deviceId + passphraseState
    Note over App: Persist deviceId + passphraseState only
    App->>SDK: evmGetAddress(..., passphraseState)
    SDK->>Device: Same hidden wallet
    SDK-->>App: { address }
    App->>SDK: btcGetAddress(..., passphraseState)
    SDK->>Device: Same hidden wallet, no new prompt
    SDK-->>App: { address }
```
