...
> **Important: device locking invalidates sessions.** Touch and Pro devices auto-lock after a timeout. Once locked, all passphrase sessions are cleared — the cached `session_id` becomes invalid and the user must re-enter their passphrase after unlocking (PIN). Session reuse only works when the device remains unlocked between invocations.
