...
### Later process / reconnect

Persist only `deviceId + passphraseState`. Do **not** persist firmware `session_id`. Load the binding and pass it on the next address / sign call. Core resumes inside that method. `openWalletSession` has no resume mode.
...
If the device locked between processes, PIN runs again on the first protected call. See [Wallet Sessions](/en/hardware-sdk/concepts/wallet-session).
...
| Storage | Stored on device | Do not persist plaintext; SDK caching depends on the input flow and `save` |
...
A newly entered passphrase can derive a different wallet. When a later call supplies an expected `passphraseState`, the SDK checks that binding and fails on a mismatch; do not treat a mistyped passphrase as the intended wallet.
