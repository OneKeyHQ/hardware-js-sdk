...
Persist only `deviceId + passphraseState`. Do **not** persist firmware `session_id`. Load the binding and pass it on the next address / sign call. Core resumes inside that method. Do not call `openWalletSession({ mode: 'resume-hidden' })`.
