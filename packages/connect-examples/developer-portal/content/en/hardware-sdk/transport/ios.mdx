...
extension ViewController: CBCentralManagerDelegate, CBPeripheralDelegate {
  ...
  func peripheral(_ p: CBPeripheral, didUpdateValueFor ch: CBCharacteristic, error: Error?) {
    ...
    // Forward hex to JS — queue raw chunks for receive(uuid); the SDK reassembles frames
...
- Persist `connectId` (peripheral UUID) and fetch `deviceId` with `getDeviceState(connectId)` after the first connection.
...
- BLE chunks and V1/V2 framing: [See Low-Level Transport Plugin](/en/hardware-sdk/concepts/low-level-plugin#ble-protocol-native-mobile)
