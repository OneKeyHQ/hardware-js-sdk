---
title: deviceSettings
---

# deviceSettings

## Device settings

It is recommended to modify only one property when using it, as it needs to be confirmed on the device.

```typescript
const result = await HardwareSDK.deviceSettings(connectId, params);
```

## Params

[**Optional common params**](/en/hardware-sdk/concepts/common-params)

* `language` - _optional_ `string` Device language, [ISO 639-1 codes](https://en.wikipedia.org/wiki/List\_of\_ISO\_639-1\_codes)
* `label` - _optional_ `string` Device name
* `usePassphrase` - _optional_ `boolean` Open passphrase or not
* `homescreen` - _optional_ `string` Change homescreen by hex string (Protocol V1 only)
* `autoLockDelayMs` - _optional_ `number` Auto lock time in ms
* `displayRotation` - _optional_ `number` In degrees from North (Protocol V1 only)
* `passphraseAlwaysOnDevice` - _optional_ `boolean` Do not prompt for passphrase, enforce device entry (Protocol V1 only)
*   `safetyChecks` - _optional_ `number` Enum_SafetyCheckLevel numeric value (Protocol V1 only)

    0: Strict
    1: PromptAlways
    2: PromptTemporarily

* `experimentalFeatures` - _optional_ `boolean` Enable experimental message types (Protocol V1 only)

## Example

```typescript
HardwareSDK.deviceSettings(connectId, {
    label: 'My OneKey Wallet'
});
```

## Result

```typescript
{
    success: true,
    payload: {
        message: string
    }
}
```

Error

```typescript
{
    success: false,
    payload: {
        error: string, // error message
        code?: string | number // error code
    }
}
```
