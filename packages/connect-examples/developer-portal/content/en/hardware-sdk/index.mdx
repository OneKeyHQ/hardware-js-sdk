...
Hardware SDK **1.2.0** is the current stable line. It speaks Protocol V1 and Protocol V2 through the same JavaScript API. New here? Start with [Getting Started](/en/hardware-sdk/getting-started).
...
| **Bluetooth BLE (native)** | Native apps over BLE (not Web Bluetooth); requires OS Bluetooth & permission | Pro, Pro 2, Neo, Touch, Classic 1s / Classic 1s Pure; **Mini does not support BLE** |
| **Air-Gap (QR)** | Offline QR signing | OneKey Pro and Pro 2; **Neo has no camera** |
...
Use `@onekeyfe/hd-web-sdk` (plugin-only). The SDK default iframe host is `https://jssdk.onekey.so/<installed-package-version>/` from `getSDKVersion()`. Omit `connectSrc` unless you host the iframe yourself.

import { SdkVersionNote } from '../../../components/SdkVersionNote'

<SdkVersionNote locale="en" />
...
After `HardwareSDK.init(...)` returns, follow [Getting Started](/en/hardware-sdk/getting-started) for the first address, then the [Core API Guide](/en/hardware-sdk/core-api-guide) for the rest of the flow:

- 1.2.0 device matrix: [Devices](/en/hardware-sdk/concepts/devices)
- Which chains on Pro 2 / Neo: [Chain Support](/en/hardware-sdk/concepts/chain-support)
- Protocol V1 vs V2 (auto-detected): [Protocol](/en/hardware-sdk/concepts/protocol)
- Open / resume a wallet: [Wallet Sessions](/en/hardware-sdk/concepts/wallet-session)
- Transport chooser and reconnect: [Transport](/en/hardware-sdk/transport) · [Reconnect](/en/hardware-sdk/transport/reconnect)
- Firmware V2 / V3 / V4: [Firmware Update](/en/hardware-sdk/firmware-update)

- Standard lifecycle (`init → search → getDeviceState → subscribe → call + uiResponse → cleanup`): [Core API Guide → Standard lifecycle](/en/hardware-sdk/core-api-guide#standard-lifecycle)
- Call parameters accepted by every method: [Common params](/en/hardware-sdk/concepts/common-params)
