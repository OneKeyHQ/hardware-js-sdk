...
- For repeated calls, serialize per device and reuse `passphraseState` (or `useEmptyPassphrase: true`). `keepSession` is deprecated.
...
- Repeated passphrase prompts: call `openWalletSession` once, then pass `passphraseState` on later calls. Serialize per device. `keepSession` is deprecated.
