...
- Business APIs mostly unchanged: e.g., `searchDevices / getDeviceState / btcGetAddress`
...
const { connectId } = list.payload[0] ?? {};
if (!connectId) throw new Error("No device connection");

// 2) Confirm deviceId
const state = await HardwareSDK.getDeviceState(connectId);
if (!state.success) throw new Error(state.payload.error);
const resolvedDeviceId = state.payload.identity.deviceId;
if (!resolvedDeviceId) throw new Error("Device has no wallet identity");
...
- API references and params: [Hardware SDK API References](/en/hardware-sdk/core-api-guide), [Common Params](/en/hardware-sdk/concepts/common-params), [Error Codes](/en/hardware-sdk/core-api-guide#error-codes)
