...
- Prefer omitting `connectSrc` so `@onekeyfe/hd-web-sdk` fills `https://jssdk.onekey.so/<installed-version>/`. Override only if you host the iframe yourself.
...
await HardwareSDK.init({
  env: 'webusb',
  // omit connectSrc — SDK fills https://jssdk.onekey.so/<installed-hd-web-sdk-version>/
  debug: true,
  fetchConfig: true,
});
```
...
- Omit `connectSrc` unless you host the iframe; it must match the installed `@onekeyfe/hd-web-sdk` version
