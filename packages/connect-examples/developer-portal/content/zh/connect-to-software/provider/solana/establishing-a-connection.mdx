...
`connect()` 调用将返回一个 [Promise](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Promise)，当用户接受连接请求时解析，当用户拒绝请求或关闭弹窗时拒绝（await 时抛出）。有关 OneKey 可能发出的错误消息的详细信息，请参阅[错误](./signing-a-message#errors)。
