...
应用[检测到 Provider](/zh/connect-to-software/provider/solana/detecting-the-provider) 后，就可以请求连接到 OneKey。此连接请求将提示用户授权分享其公钥，表明他们愿意进一步交互。用户必须先批准连接请求，应用才能发出其他请求，如[签署消息](/zh/connect-to-software/provider/solana/signing-a-message)或[发送交易](/zh/connect-to-software/provider/solana/sending-a-transaction)。
...
`connect()` 调用将返回一个 [Promise](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Promise)，当用户接受连接请求时解析，当用户拒绝请求或关闭弹窗时拒绝（await 时抛出）。有关 OneKey 可能发出的错误消息的详细信息，请参阅[错误](/zh/connect-to-software/provider/solana/signing-a-message#errors)。
