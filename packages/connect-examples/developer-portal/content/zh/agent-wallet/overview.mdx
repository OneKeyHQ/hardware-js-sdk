...
当前实现基于 `apps/cli` 中的 `@onekeyfe/cli` 包，以及外部 [`onekey-wallet-skills`](https://github.com/OneKeyHQ/onekey-wallet-skills) 仓库。CLI 包名是 `@onekeyfe/cli`，可执行命令是 `onekey`。

核心流程：

- `onekey auth login --app-transfer`：配对 OneKey App Agent Wallet 会话。
- `onekey auth login --hardware`：启动已连接的 OneKey 硬件钱包会话。
- `onekey schema --list` 和 `onekey schema <command>`：暴露机器可理解的命令契约。
- `onekey auth status`：确认当前活动会话的事实来源。
- `onekey balance`、`onekey transfer`、`onekey swap`、`onekey token`、`onekey market`、`onekey security`：提供业务能力面。
