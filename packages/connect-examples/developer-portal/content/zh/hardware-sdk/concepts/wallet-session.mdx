...
现有应用可以继续用 [`getPassphraseState`](/zh/hardware-sdk/basic-api/get-passphrase-state)，并在每次调用上带 `initSession` / `useEmptyPassphrase` / `passphraseState`。Core 会把它映射到 Protocol V1 和 V2。
