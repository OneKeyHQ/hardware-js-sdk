...
设备初始化 Passphrase Session 时临时生成的随机 token。固件会将它返回给
Core，但当前公共钱包会话响应和公共 Features 投影都不暴露该值。

- **锁屏即失效**（自动锁、手动锁、USB 断开）。
- SDK 内部用它维持设备端缓存的 passphrase。
- 不暴露给新的 App 集成；应用只需要 `deviceId + passphraseState`。
...
- 作为 `passphraseState` 通用参数传给后续所有需要绑在这个隐藏钱包上的 SDK
  调用。
- 持久化 `deviceId + passphraseState` 作为钱包身份。新进程应显式重新打开钱包，
  不持久化 SDK 内部 Session ID。
...
- `passphraseState` 标识**是哪一个隐藏钱包**；`session_id` 标识**这条还活着
  的会话**。
- 新集成持久化 `deviceId + passphraseState` 作为钱包身份，并在每个新进程中
  显式打开钱包。`session_id` 由 SDK 内部管理。
