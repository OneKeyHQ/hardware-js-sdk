...
| 类别 | 适用场景 | 代表方法 |
| --- | --- | --- |
| 🟡 仅 `connectId` | 设备级操作（不触碰钱包状态）| `searchDevices` · `getDeviceState` · `openWalletSession` · `deviceLock` · `deviceUnlock` · `deviceWipe` · `deviceSettings` · `deviceVerify` · `cancel` · `checkFirmwareRelease` · `checkBootloaderRelease` |
...
> ⚠️ 第三行是最坑的：隐藏钱包调用时漏传 `passphraseState`，拿到的是标准钱包的地址/签名，**完全没有报错**。所以隐藏钱包场景里，用 `openWalletSession` 拿一次 `passphraseState` 之后必须挂到后续所有钱包级调用上；如果明确就是要走标准钱包，就显式传 `useEmptyPassphrase: true`，别把它留空。
...
- 要留在隐藏钱包上，后续方法带已持久化的 `passphraseState`。Core 会恢复固件
  session。不要为此再调 `openWalletSession({ mode: 'resume-hidden' })`。
