...
> `useEmptyPassphrase` 等）？请看 [通用参数](../common-params)。
...
- [通用参数](../common-params) —— 传**进** SDK 的参数
- [Passphrase](./passphrase) —— 隐藏钱包流程
- [PIN 码](./pin) —— PIN 输入流程
- [Search Devices](../basic-api/search-devices) —— 返回 `connectId`、`uuid`、`deviceType`、`deviceId`
- [Get Features](../basic-api/get-features) —— 返回 `device_id`、`session_id`、`onekey_serial` 等
