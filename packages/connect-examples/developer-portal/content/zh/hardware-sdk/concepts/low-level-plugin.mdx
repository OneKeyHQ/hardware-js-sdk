...
CRC8 是 Protocol V2 协议必需字段，不是设备数据。使用 `@onekeyfe/hd-common-connect-sdk` 的 `lowlevel` 环境时，JS SDK 会负责计算和校验 CRC8；原生 Kotlin/Swift/Flutter 插件只需要把 BLE notification 按 `LenL/LenH` 重组成完整 `0x5A` frame，并把完整 frame 的 hex 字符串从 `receive()` 返回。不要去掉 header、payload 或末尾 CRC。
...
| 填充 | 零填充到 64 字节 | 无需填充 |
| `receive()` 返回 | 每次一个 64 字节帧 | **Protocol V1 完整消息，或 Protocol V2 完整 `0x5A` frame** |
...
如果 notification 的第一个字节是 `0x5A`，或当前已经处于 V2 frame 缓冲中，就按 Protocol V2 处理：
...
```typescript
// ❌ 错误: 返回单个 BLE notification 或重新分片成 64 字节包
receive() { return this.messageQueue.shift() }

// ✅ 正确: 返回完整重组后的 V1 消息或 V2 frame
