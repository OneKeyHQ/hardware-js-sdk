...
原始设备 Session ID 是 SDK 内部状态，新的公共钱包会话响应不再返回该值。新的 CLI 或脚本不应持久化或重建它；应在每个新进程中显式打开钱包，再使用该进程获得的 `deviceId + passphraseState` 执行后续调用。

`preloadSessionCache` 仅为旧版受信任 CLI 迁移其先前已保存在操作系统 Keychain 中的完整 Session 三元组而保留。不要将它作为新的 App、网页、插件或文件存储接入点。

> **重要：设备锁定会使会话失效。** 所有 OneKey 设备在锁定时（无论是自动锁屏、手动锁定还是 USB 断开）都会清除 Passphrase 会话。解锁后应重新打开钱包，而不是重放旧 Session ID。

### 先解锁模式（CLI 推荐）

对于可能自动锁屏的设备（Touch/Pro），始终先解锁，再显式打开钱包：
