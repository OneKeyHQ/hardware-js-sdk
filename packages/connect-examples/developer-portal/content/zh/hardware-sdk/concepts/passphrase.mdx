...
应用只持久化 `deviceId + passphraseState`。临时 `session_id` 只保存在 SDK
进程内的钱包 Session Store 中；SDK 进程和设备 Session 仍有效时，后续业务调用会自动恢复。

应用接口不要读取、持久化或传入 `sessionId`。进程重启、设备锁定或 Session
失效后，应重新打开钱包，让用户完成正常 Passphrase 流程。
