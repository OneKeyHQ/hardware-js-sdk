...
1. 应用调用 SDK 方法并设置 `useEmptyPassphrase: true`
2. SDK 使用空 passphrase 在设备上初始化会话
3. 设备直接返回结果——无需用户操作
...
1. 应用调用 SDK 方法（不设置 `useEmptyPassphrase`）
2. SDK 触发 `UI_REQUEST.REQUEST_PASSPHRASE` 事件
3. 应用显示 Passphrase 对话框，提供两个选项：设备端输入或软件端输入
4. 应用调用 `uiResponse` 传入用户的选择
5. SDK 将 passphrase 发送到设备并返回结果
...
1. 调用 `getPassphraseState` — 仅提示用户一次，返回 `passphraseState`
2. 后续调用传入 `passphraseState` + `initSession: true` — 不再提示
