...
当前稳定线是 Hardware SDK **1.2.0**。Protocol V1 和 Protocol V2 走同一套 JavaScript API。第一次接入请先看 [快速开始](/zh/hardware-sdk/getting-started)。
...
| **Bluetooth BLE（原生）** | 原生 App 的 BLE；不支持 Web Bluetooth | Pro、Pro 2、Neo、Touch、Classic 1s / Classic 1s Pure；**Mini 不支持 BLE** |
| **Air-Gap（二维码）** | 离线扫码签名 | OneKey Pro 和 Pro 2；**Neo 无摄像头** |
...
使用 `@onekeyfe/hd-web-sdk`（仅插件端）。SDK 默认 iframe 地址是 `https://jssdk.onekey.so/<安装包版本>/`，来自 `getSDKVersion()`。除非自建 iframe，否则不要传 `connectSrc`。

import { SdkVersionNote } from '../../../components/SdkVersionNote'

<SdkVersionNote locale="zh" />
...
await HardwareSDK.init({
  debug: process.env.NODE_ENV !== 'production',
  // 省略 connectSrc — SDK 会填 https://jssdk.onekey.so/<安装版本>/
});
```

## 下一步

`HardwareSDK.init(...)` 完成之后，先用 [快速开始](/zh/hardware-sdk/getting-started) 读出第一个地址，其余流程在 [Core API Guide](/zh/hardware-sdk/core-api-guide) 里端到端讲清楚：

- 1.2.0 机型矩阵：[设备](/zh/hardware-sdk/concepts/devices)
- Pro 2 / Neo 支持哪些链：[链支持](/zh/hardware-sdk/concepts/chain-support)
- Protocol V1 / V2（自动识别）：[协议](/zh/hardware-sdk/concepts/protocol)
- 打开 / 恢复钱包：[钱包会话](/zh/hardware-sdk/concepts/wallet-session)
- 传输选择和重连：[传输](/zh/hardware-sdk/transport) · [重连](/zh/hardware-sdk/transport/reconnect)
- 固件 V2 / V3 / V4：[固件升级](/zh/hardware-sdk/firmware-update)
- 标准生命周期（`init → search → getDeviceState → subscribe → call + uiResponse → cleanup`）：[Core API Guide → 标准生命周期](/zh/hardware-sdk/core-api-guide#standard-lifecycle)
- 所有方法都接受的调用参数：[通用参数](/zh/hardware-sdk/concepts/common-params)
