...
React Native BLE transport 会在连接后自动探测 Protocol V1 / Protocol V2。Protocol V2 设备使用 `0x5A` frame、BLE UART router 和 CRC8 校验；这些 encode/decode、frame 重组和 CRC8 校验都由 SDK 内部处理，应用层不需要实现 CRC8 查表或手动选择协议。
...
- 不要在 React Native 应用层重写 BLE 分包或 CRC8。只要使用 `@onekeyfe/hd-ble-sdk`，Protocol V2 的 `0x5A` frame 和 CRC8 都由 transport 层维护。
