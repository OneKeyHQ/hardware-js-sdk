...
extension ViewController: CBCentralManagerDelegate, CBPeripheralDelegate {
  ...
  func peripheral(_ p: CBPeripheral, didUpdateValueFor ch: CBCharacteristic, error: Error?) {
    ...
    // 将十六进制转发给 JS — 原始块入队供 receive(uuid) 读取，由 SDK 重组帧
...
- 持久化 `connectId`（外设 UUID）并在首次连接后使用 `getDeviceState(connectId)` 获取 `deviceId`。
- 始终在 JS 中订阅 `UI_EVENT` 以避免请求停滞（参见 [事件配置](/zh/hardware-sdk/core-api-guide#events-ui)）。
- 将 `web/web_dist/` 保留在您的应用包中并相应调整 `load` 路径。

## 参考

- BLE 数据块与 V1/V2 帧处理：[见底层传输插件](/zh/hardware-sdk/concepts/low-level-plugin#ble-协议原生移动端)
