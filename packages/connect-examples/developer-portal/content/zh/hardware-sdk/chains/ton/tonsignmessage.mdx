...
* 固件版本要求
  * Touch: 4.10.0
  * Pro 2 / Neo：Protocol V2（同一公开方法，SDK 负责字段映射）
...
```typescript
{
    ...
    payload: {
        ...
        signature: string;
        signing_message?: string; // 规范字段。Pro2/Neo 返回这个；SDK 也会从 V1 的拼写错误字段拷过来
        signning_message?: string; // 已废弃的 Protocol V1 拼写。新代码不要读它
    }
}
```
