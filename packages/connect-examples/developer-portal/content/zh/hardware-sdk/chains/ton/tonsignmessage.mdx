...
使用指定 BIP32 路径派生的私钥对消息或交易进行签名。用户需要在 OneKey 设备上确认操作。
...
* `jettonWalletAddress` - _可选_ `string` Jetton 钱包合约地址
...
// Jetton 转账
const jettonTransferResponse = await HardwareSDK.tonSignMessage(connectId, deviceId, {
    path: "m/44'/607'/0'",
    destination: 'UQBYkuShkZzRYAWX_HrK3kFpeAixiRKd-K7QBXYxl9OBXM0_',
    tonAmount: 100,
    seqno: 0,
    ...
    jettonWalletAddress: 'UQBYkuShkZzRYAWX_HrK3kFpeAixiRKd-K7QBXYxl9OBXM0_',
    jettonAmount: 101,
    fwdFee: 100,
