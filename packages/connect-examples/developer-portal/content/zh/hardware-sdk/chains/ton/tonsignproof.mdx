...
使用指定 BIP32 路径派生的私钥为 dApp 身份验证签名证明消息。用户需要在 OneKey 设备上确认操作。
...
* `expireAt` - _必需_ `number` Unix 秒单位的签名时间戳；虽然字段名为 expireAt，固件实际将其放入 TON proof 的 timestamp 字段
...
`expireAt` 会成为签名内容中的 `timestamp`，SDK 不转换时间单位。参见 [TON Connect proof 格式](https://github.com/ton-blockchain/ton-connect/blob/main/spec/connect.md#address-proof-signature-ton_proof)。
