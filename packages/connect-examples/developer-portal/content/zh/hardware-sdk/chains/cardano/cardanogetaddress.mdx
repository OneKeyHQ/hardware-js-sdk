...
显示指定 BIP32-Ed25519 路径派生的地址，并返回给调用者。用户需要在 OneKey 上确认导出。
...
* `showOnOneKey` - _必需_ `boolean` 决定是否在设备上显示地址。默认设置为 `true`
...
* `addressType` - _必需_ `CardanoAddressType`/`number` - 可以使用 flow `PROTO.CardanoAddressType` 对象或 typescript `CardanoAddressType` 枚举。支持所有地址类型。
* `path` - `string | Array<number>` 使用密钥支付地址时必填；脚本支付地址或奖励地址省略。提供时 SDK 校验至少 `3` 级。
* `stakingPath` - _可选_ `string | Array<number>` SDK 校验至少 `3` 级，用于基础地址和奖励地址派生
...
当前 TypeScript 接口要求显式提供这些字段；运行时默认值为 ICARUS 和 `showOnOneKey: true`。
