...
> [标识符（Identifiers）](./concepts/identifiers)。
...
- [标识符（Identifiers）](./concepts/identifiers) —— 响应里的 ID 字段（`connectId`、`deviceId`、`passphraseState`、`session_id`、`uuid`、`serial_no`）
- [Passphrase](./concepts/passphrase) —— 隐藏钱包完整流程
- [PIN 码](./concepts/pin) —— PIN 输入流程
