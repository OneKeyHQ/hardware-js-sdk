...
`DEVICE.CONNECT` 和 `DEVICE.DISCONNECT` 的 `payload.device` 在所有 transport
（包括 React Native BLE 和 `lowlevel`）中都是可序列化的 `KnownDevice` 快照，而不是
SDK 内部的实时 `Device` 实例。请使用 `connectId` 调用后续 SDK 方法，使用 `serialNo`
识别物理设备；`uuid` 仅为废弃兼容别名。不要调用 `device.run()`、`acquire()`、
`release()` 或 `commands`，也不要保存事件对象并期待字段自动变化。状态变化请监听
`DEVICE.STATE`；Protocol V1 兼容业务也可以监听 `DEVICE.FEATURES`。
