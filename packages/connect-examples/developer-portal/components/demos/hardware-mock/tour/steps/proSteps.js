...
function createProRequestButtonHintSteps(locale, { command, showOnOneKey }) {
  ...
  return [
    hintStep({
      ...
      desc: isEn
        ? `SDK emits \`UI_EVENT: REQUEST_BUTTON\`. ${
            isGetAddress && showOnOneKey
              ? 'Device shows address for confirmation.'
              : 'Device awaits user confirmation.'
          }`
        : `SDK 触发 \`UI_EVENT: REQUEST_BUTTON\`。${
            isGetAddress && showOnOneKey ? '设备显示地址等待确认。' : '设备等待用户确认。'
          }`,
    }),
  ];
}
