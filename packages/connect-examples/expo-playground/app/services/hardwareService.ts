...
export type { TransportType } from '../utils/hardwareInstance';
// 使用 hd-core 的标准类型
...
const extractPassphraseStateMetadata = (payload: unknown): PassphraseStateMetadata => {
  ...
  const maybeWalletType = (payload as { walletType?: unknown }).walletType;
  ...
  return {
    ...
    walletType:
      maybeWalletType === 'standard' || maybeWalletType === 'hidden' ? maybeWalletType : undefined,
  };
};
...
const preparePassphraseParams = async (
  ...
  try {
    ...
    if (passphraseMetadata.walletType !== 'hidden') {
      throw new Error('Hidden-wallet selection returned a non-hidden walletType.');
    }
