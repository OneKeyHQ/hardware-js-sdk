...
const mockDeviceStatusGet = jest.fn(async (...args: unknown[]) => {
  void args;
  return {
    success: true,
...
describe('callHardwareAPI', () => {
  ...
  test('不使用非空 passphraseState 推断标准钱包为隐藏钱包', async () => {
    mockProtocol = 'V2';
    mockPassphraseProtection = true;
    mockOpenWalletSession.mockResolvedValueOnce({
      success: true,
      payload: {
        protocol: 'V2',
        walletType: 'standard',
        deviceId: 'device-id',
        passphraseState: 'standard-wallet-state',
        resumed: false,
      },
    });
    Object.defineProperty(globalThis, 'window', {
    ...
    const result = await callHardwareAPI('evmGetAddress', {
      connectId: 'connect-id',
      deviceId: 'device-id',
      path: "m/44'/60'/0'/0/0",
    });
    ...
    expect(result).toMatchObject({
      success: false,
      payload: {
        error: expect.stringContaining('non-hidden walletType'),
      },
    });
    expect(mockEvmGetAddress).not.toHaveBeenCalled();
  });
...
describe('callHardwareAPI Protocol V2 development methods', () => {
  test('通过公开 CoreApi 调用 Pro2 原生命令', async () => {
    Object.defineProperty(globalThis, 'window', {
      configurable: true,
      value: {},
    });
    ...
    expect(mockDeviceStatusGet).toHaveBeenCalledWith('connect-id', {
      connectId: 'connect-id',
    });
  });
});
