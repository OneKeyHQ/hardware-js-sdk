...
export const WALLET_SESSION_CASES: WalletSessionCaseDefinition[] = [
  ...
  {
    id: 'attach-pin-switch-standard',
    title: 'Attach PIN 上下文切换到标准钱包',
    description: '验证标准钱包请求通过 Main PIN 流程重置当前钱包上下文，并取得标准钱包 Session。',
    category: 'attach-pin',
    protocols: BOTH_PROTOCOLS,
    prerequisites: ['attach-pin-state'],
    steps: ['调用 openWalletSession(standard)', '统计 Passphrase UI 请求数'],
    expected: ['返回标准钱包', 'walletType=standard', '不得弹出钱包选择'],
    execution: 'automatic',
    sdkMethod: 'openWalletSession',
  },
  {
    id: 'attach-pin-reselect-after-standard',
    title: '标准钱包切换后重新进入 Attach PIN 钱包',
    description: '从标准钱包重新选择同一个 Attach PIN 钱包，为钱包状态错配恢复检查准备上下文。',
    category: 'attach-pin',
    protocols: BOTH_PROTOCOLS,
    prerequisites: ['attach-pin-switch-standard'],
  ...
  {
    id: 'attach-pin-wrong-wallet-recovery',
    title: 'Attach PIN 上下文恢复目标隐藏钱包',
    description:
      '请求钱包 A，但当前 Attach PIN 指向另一个隐藏钱包时，通过统一钱包选择重新进入钱包 A。',
    category: 'attach-pin',
    protocols: BOTH_PROTOCOLS,
    prerequisites: ['attach-pin-reselect-after-standard', 'hidden-a-address'],
    steps: [
      '确认 Attach PIN 钱包不同于钱包 A',
      '用钱包 A 的 passphraseState 请求地址',
      '在统一钱包选择中重新进入钱包 A',
    ],
    expected: ['只弹出一次钱包选择', '返回钱包 A 地址', '不返回当前 Attach PIN 钱包地址'],
    execution: 'interactive',
    sdkMethod: 'evmGetAddress',
  },
  {
  ...
  },
  {
    ...
    description:
      '普通重连后显式恢复钱包 A；Session 有效时直接恢复，状态错配时只允许交互式重选钱包 A。',
    category: 'identity',
    protocols: BOTH_PROTOCOLS,
    prerequisites: ['reconnect-same-device', 'hidden-a-address'],
    steps: [
      '调用 openWalletSession(resume-hidden)',
      '如弹出钱包选择则重新进入钱包 A',
      '比较钱包身份与地址',
    ],
    expected: [
