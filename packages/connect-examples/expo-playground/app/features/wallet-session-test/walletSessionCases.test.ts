...
describe('wallet session WebUSB test matrix', () => {
  test('covers wallet identity, cache isolation, Attach PIN, reconnect and reset flows', async () => {
    ...
    expect(ids).toEqual(
      new Set([
        ...
        'attach-pin-select',
        'attach-pin-state',
        'attach-pin-switch-standard',
        'attach-pin-reselect-after-standard',
        'attach-pin-wrong-wallet-recovery',
  ...
  test('makes restart, reconnect, global clear and Attach PIN safety expectations executable', async () => {
    ...
    expect(byId.get('reconnect-session-outcome')?.expected.join(' ')).toContain('只弹出一次');
    expect(byId.get('all-cache-invalid')?.expected.join(' ')).toContain('WalletSessionInvalid');
    expect(byId.get('attach-pin-switch-standard')?.expected.join(' ')).toContain(
      'walletType=standard'
    );
    expect(byId.get('attach-pin-wrong-wallet-recovery')?.expected.join(' ')).toContain(
      '只弹出一次钱包选择'
    );
