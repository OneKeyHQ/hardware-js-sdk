    ...
    <section className="grid shrink-0 grid-cols-1 gap-px bg-border lg:h-[clamp(300px,32vh,360px)] xl:grid-cols-[minmax(380px,0.48fr)_minmax(0,0.52fr)]">
      <div className="flex min-h-0 flex-col overflow-hidden bg-muted/20">
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-3 2xl:grid-cols-4">
            {summary.map(item => (
          ...
          <ul className="flex flex-wrap gap-x-5 gap-y-2 border-t border-border/60 pt-3 text-xs text-muted-foreground">
      ...
      <div className="flex min-h-0 min-w-0 flex-col overflow-hidden bg-background">
        ...
        <div className="min-h-0 flex-1 p-4">
          {state.status === 'idle' ? (
            <div className="flex h-full min-h-32 items-center justify-center rounded-xl border border-dashed border-border px-6 text-center text-sm text-muted-foreground">
              执行后将在这里显示地址、签名或已签交易。
            </div>
          ) : null}
          {state.status === 'running' ? (
            <div className="flex h-full min-h-32 items-center justify-center rounded-xl border border-primary/30 bg-primary/5 px-6 text-center text-sm">
          ...
          {state.status === 'success' ? (
            <pre className="h-full min-h-0 overflow-auto whitespace-pre-wrap break-all rounded-xl border border-border bg-muted/30 p-4 font-mono text-xs leading-5">
