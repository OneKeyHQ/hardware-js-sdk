...
describe('SDKProvider canonical device state events', () => {
  test('stores DEVICE.STATE and applies it to the connected device', () => {
    expect(source).toContain('sdkInstance.on(DEVICE_EVENT');
    expect(source).toContain('message.type === DEVICE.STATE');
    expect(source).toContain('setDeviceState(message.payload.state)');
  });
});
